...
type JSONSchema2TS<T> =
  ...
  T extends { enum: infer U } ? U extends unknown[] ? TupleToUnion<U> : never : number :
  T extends { type: 'string' } ?
  T extends { enum: infer U } ? U extends unknown[] ? TupleToUnion<U> : never : string :
  // object types
  T extends { type: 'object' } ?
  T extends { properties: infer U } ?
  { [K in keyof U]?: JSONSchema2TS<U[K]> } :
  T extends { required: infer U } ?
  U extends unknown[] ?
  { [K in U[number]]: JSONSchema2TS<T['properties'][K]> } & Record<string, unknown> :
  never :
  Record<string, unknown> :
  // array types
  T extends { type: 'array' } ?
  T extends { items: infer U } ? JSONSchema2TS<U>[] : unknown[] :
  never
